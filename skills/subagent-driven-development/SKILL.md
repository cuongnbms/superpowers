...
description: Executes a written implementation plan in the current session with a fresh subagent per task, a spec-and-quality review after each task, and a whole-branch review at the end. Use when the user has an implementation plan (a plans/*.md file) and asks to implement, execute, or run it task by task. Not for writing the plan itself or for fixing a single bug.
---
...
Setup, then per task: dispatch an implementer, review, run the fix loop if the review is not clean, complete the task. After the last task: final whole-branch review, one fix wave, finish.
...
execution begins — each finding against the plan text that mandates it,
the spec deciding — record the ruling beside its row, and dispatch Task 1.
...
- **Task brief:** before dispatching an implementer, run this skill's
  ...
  so the brief stays the single source of requirements. Your dispatch
  ...
  strings, signatures, test cases) appear only in the brief. Never make a
...
**DONE_WITH_CONCERNS:** The implementer completed the work but flagged doubts. Read the concerns before proceeding. If a concern is about correctness or scope, rule on it, ledger the ruling, and resume the implementer with it before review — you do not fix code yourself. If they're observations (e.g., "this file is getting large"), note them in the ledger and proceed to review.
...
- **Reviewer inputs:** the task reviewer gets three paths — the same brief
  file, the report file, and the review package. The brief already
  carries the plan's Global Constraints, so implementer and reviewer are
  bound by the same text.
- The spec-constraints block is for requirements the spec states and the
  plan omitted: exact values, exact formats, stated relationships between
  components ("same layout as X", "matches Y"). Copy them verbatim from
  the spec. Leave the block empty when the plan's section covers the
  task. The reviewer's template already carries the process rules (YAGNI,
  test hygiene, review method).
...
- A finding labeled plan-mandated — or any finding that conflicts with
  ...
  against the plan text, the spec deciding, and ledger the ruling before
  you act on it. Do not dismiss the finding because the plan mandates it,
  and do not dispatch a fix that contradicts the plan without a recorded
  ruling.
...
`minor (deferred)` lines the final review did not force before merge — the
same silent-discard rule applies to them.
...
Task reviewer: Spec ❌:
  ...
  Issues (Important): repair mode swallows write errors (src/recovery.js:58)
  Issues (Minor): magic number 100 for the progress interval

[Ledger: Task 2: minor (deferred): magic number 100 in src/recovery.js]
[Fix round 1: resume the implementer with the two blocking findings]
Implementer: Added progress reporting, repair mode now surfaces write errors.
  Re-ran test/recovery.test.js — 10/10 passing. Fix report appended.

[Run review-package PLAN_FILE FIX_BASE HEAD; dispatch scoped re-review]
Re-reviewer: Missing progress reporting — ADDRESSED (src/recovery.js:41).
  Swallowed write errors — ADDRESSED (src/recovery.js:58). New breakage: none.
